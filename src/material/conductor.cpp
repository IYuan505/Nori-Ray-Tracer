...
struct conductorCoeff {
    ...
    // the constructor
    conductorCoeff(const std::string name, const Color3f eta, const Color3f k)
        : name(name), eta(eta), k(k) { }
};

static const conductorCoeff matrialList[] = {
...
/// Smooth conductor BSDF
class Conductor : public BSDF {
public:
    Conductor(const PropertyList &propList) {
        m_material = propList.getString("material", "null");
        if (m_material == "null") {
            m_eta = propList.getColor("eta", 1.0f);
            m_k = propList.getColor("k", 1.0f);
        } else {
            bool found = false;
            for (auto material: matrialList) {
                if (material.name == m_material) {
                    found = true;
                    m_eta = material.eta;
                    m_k = material.k;
                }
            ...
            if (!found)
                throw NoriException("Specified material not found!");
        }
    }
    ...
    Color3f sample(BSDFQueryRecord &bRec, const Point2f &sample) const {
        ...
        return frConductor(Frame::cosTheta(bRec.wi));
    }

    Color3f frConductor(float cosThetaI) const {
        return Color3f(fresnel_conductor(cosThetaI, m_eta[0], m_k[0]),
                       fresnel_conductor(cosThetaI, m_eta[1], m_k[1]),
                       fresnel_conductor(cosThetaI, m_eta[2], m_k[2]));
    }
